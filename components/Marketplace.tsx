...
export default function Marketplace({
  ...
  // Call onBasketChange when basket items change
  useEffect(() => {
    if (onBasketChange) {
          ...
          {showHeader && (
            <div className="mb-8 text-center">
              <div className="flex flex-col items-center gap-2">
                <span className="inline-block bg-green-100 text-green-800 text-sm font-medium px-4 py-1 rounded-full mb-2">
                  {title || t('title')}
                </span>
                <div className="flex items-center justify-center gap-2">
                  <Sparkles className="w-8 h-8 text-green-800" />
                  <h1 className="text-4xl md:text-5xl font-extrabold text-green-800 mb-0">
                    {title || t('createBasket')}
                  </h1>
                </div>
